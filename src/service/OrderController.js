...
class OrderController {
  ...
  static async buy(req, response) {
    ...
    dmc_client
      ...
      .then(async (res) => {
        ...
        // 保存订单到neDB
        var saveRes = await orderService.saveOrder(
          email,
          orderBasic.orderId,
          orderBasic.miner,
          orderBasic.user,
        ...
        // 同步订单到注册中心
        var syncOrderRes = await orderService.syncOrder2RegisterCenter(
          email,
          orderBasic.orderId,
          billId,
          unmatchedAmount * 1024 * 1024 * 1024,
          0,
          res.transaction_id
        );
        if (syncOrderRes instanceof BizResultCode) {
          response.send(
            BizResult.fail(BizResultCode.SYNC_ORDER_2_REGISTER_CENTER_FAILED)
          );
          return;
        }
   ...
   * @returns 订单列表
   */
  static async outstandingOrders(
    email,
    unmatchedAmount,
    period,
    minPrice,
    ...
    let body =
      ...
      "        }\n" +
      "    }";
    let outstandingOrders = JSON.parse(
      request("POST", transactionAddress + getOutstandingOrders, {
        headers: {
  ...
  static async orderList(email, pageNum, limit, res) {
    ...
    let orderList = JSON.parse(
      request("POST", transactionAddress + getOrders, {
        headers: {
          "Content-Type": "application/graphql",
        },
        body: body,
  ...
  static async pushMerkle(req, res) {
    ...
    powClient.GetMerkleRoot(getMerkleRequest, function (err, data) {
      ...
      logger.info(
        "pushMerkle orderId{},merkleRoot{},blockCount:{}",
        orderId,
        merkleRoot,
        dataBlockCount
      );
      dmcClient
        ...
        .then(async (result) => {
          var savePushMerkleRecordRes = await orderService.savePuskMerkleRecord(
            orderId,
            username,
            merkleRoot,
            dataBlockCount,
  ...
  static async getPushMerkleRecord(req, res) {
    ...
    var resultData = await orderService.getPuskMerkleRecord(
      orderId,
      email,
      skip,
      limit
  ...
  static async reqChallenge(req, response) {
    ...
    var codebooks = await fileService.getFileCodeBook(
      orderId.toString(),
      email,
      merkleBufferSize
    );
    ...
    var containRandomData = Buffer.concat([
      originData,
      Buffer.from(randomCharacter),
    ]);
    let preDataHash = DMC.ecc.sha256(containRandomData);
    let dataHash = Buffer.from(
      DMC.ecc.sha256(Buffer.from(preDataHash))
    ).toString("hex");
    logger.info(
      "challenge, orderId:{},cid:{},partId:{},data_hash:{},nonce:{}",
      orderId,
      codebook.cid,
      codebook.part_id,
      dataHash,
    ...
    var partId = await orderService.getPartIdByCidAndPartId(
      orderId,
      codebook.cid,
      codebook.part_id
    );
    dmc_client
      ...
      .then((res) => {
        orderService.saveChallengeRecord(
          orderId,
          email,
          codebook.cid,
          codebook.part_id,
  ...
  static async release(req, res) {
    ...
    var result = await orderService.release(
      dmc_client,
      username,
      orderId,
      amount
  ...
  static async append(req, res) {
    ...
    var result = await orderService.append(
      dmc_client,
      username,
      orderId,
      amount
  ...
  static getChainId() {
    ...
    let chainId = JSON.parse(
      request("POST", httpEndpoint + getChainInfo, {}).getBody("utf-8")
    ).chain_id;
  ...
  static getBenchmarkPrice() {
    ...
    let benchmarkPrice = JSON.parse(
      request("POST", httpEndpoint + getTableRows, {
        json: {
          json: true,
          code: "dmc.token",
          scope: "dmc.token",
